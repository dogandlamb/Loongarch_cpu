...
module EXEport (
    ...
    */
    ...
    assign br_taken        = valid && br_taken_w && !br_to_seq_w;
    assign link_pc4_w      = pc_in + 32'd4;

    // RAW stall belongs to the ID-stage consumer; the current EXE producer must
    // still drive its result/control signals so it can advance to MEM.
    ...
    assign  wb_reg_addr_out = valid && !exception_valid_w ? wb_reg_addr : 5'b0;
    assign  mem_op          = valid && !exception_valid_w ? mem_op_in : {`MEM_OP_NUM{1'b0}};
    assign  mem_wdata_out   = valid && !exception_valid_w ? mem_wdata_in : 32'b0;
    assign  wb_op           = valid && !exception_valid_w ? wb_op_in : 1'b0;

    assign data_we_from_EXE = (ex_valid && !exception_valid_w) ? 
                            (mem_op[`MEM_OP_ST_W] | mem_op[`MEM_OP_ST_B] | mem_op[`MEM_OP_ST_H])
                            : 1'b0;
    assign data_re_from_EXE = (ex_valid && !exception_valid_w) ? 
                            (mem_op[`MEM_OP_LD_W] | mem_op[`MEM_OP_LD_H] | mem_op[`MEM_OP_LD_B] | mem_op[`MEM_OP_LD_HU] | mem_op[`MEM_OP_LD_BU])
                            : 1'b0;
    ...
    assign data_wbyte_en_from_EXE = (ex_valid && !exception_valid_w) ? ((mem_op[`MEM_OP_ST_W]) ? 4'b1111 :
                                        (mem_op[`MEM_OP_ST_H]) ? ((alu_result_w[1] ? 4'b1100 : 4'b0011)) :
                                        (mem_op[`MEM_OP_ST_B]) ? (4'b0001 << alu_result_w[1:0]) : 4'b0000) : 4'b0000;

    assign csr_op_out = ex_valid ? csr_op_in : {`CSR_OP_NUM{1'b0}};
    assign csr_num_out = ex_valid ? csr_num_in : 12'b0;
    assign csr_wmask_out = ex_valid ? csr_wmask_in : 32'b0;
    assign csr_wvalue_out = ex_valid ? csr_wvalue_in : 32'b0;
    assign wb_src_op_out = ex_valid ? wb_src_op_in : {`WB_SRC_NUM{1'b0}};

    assign ertn_op_out = ex_valid ? ertn_op_in : 1'b0;
    assign sys_valid_out = ex_valid ? sys_valid_in : 1'b0;
    assign brk_valid_out = ex_valid ? brk_valid_in : 1'b0;
    assign ine_valid_out = ex_valid ? ine_valid_in : 1'b0;
    assign adef_valid_out = ex_valid ? adef_valid_in : 1'b0;
    assign int_valid_out = ex_valid ? int_valid_in : 1'b0;
    assign ale_valid_out = ex_valid ? addr_error : 1'b0;
    assign exception_valid_out = ex_valid ? exception_valid_w : 1'b0;
    assign if_vaddr_out = ex_valid && adef_valid_in ? if_vaddr_in : 32'b0;
    assign ale_vaddr_out = ex_valid && ale_valid_out ? alu_result_w : 32'b0;

    assign tlb_op_out = ex_valid ? tlb_op_in : {`TLB_OP_NUM{1'b0}};
    assign invtlb_asid_out = ex_valid ? invtlb_asid_in : 10'b0;
    assign invtlb_vpn_out = ex_valid ? invtlb_vpn_in : 19'b0;
    assign tlb_ex_valid_out = ex_valid ? tlb_ex_valid_in : {`TLB_EX_NUM{1'b0}};
    ...
    assign tlb_vaddr_out = ex_valid && tlb_ex_any_exe ? tlb_vaddr_in : 32'b0;
