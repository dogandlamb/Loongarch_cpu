...
wire sel_dc_wr = (state == S_IDLE) && (ram_port_busy !== 1'b1) && (dc_wr_req === 1'b1);
...
wire pf_can_start = (state == S_IDLE) && (ram_port_busy !== 1'b1)
                 && (pf_valid === 1'b1) && (pf_mshr_valid !== 1'b1)
                 && (dc_wr_req !== 1'b1) && (dc_rd_req !== 1'b1) && (ic_rd_req !== 1'b1);
        ...
        case (state)
            ...
            S_LOOKUP: begin
                ...
                if (req_is_read && lookup_line && lookup_hit) begin
                    ...
                    rd_issued <= 1'b0;
                    if ((req_is_prefetch === 1'b1) && (lookup_victim_dirty !== 1'b1)) begin
                        pf_mshr_valid <= 1'b1;
                        pf_mshr_issued <= 1'b0;
                        pf_mshr_addr <= req_addr;
                        pf_mshr_set <= req_set;
                        pf_mshr_tag <= req_tag;
                        pf_mshr_way <= lookup_victim_way;
                        req_is_prefetch <= 1'b0;
                        state <= S_IDLE;
                    end else if (pf_mshr_valid === 1'b1) begin
                        state <= S_WAIT_MSHR;
                    end else begin
            ...
            S_WAIT_MSHR: begin
                if ((pf_mshr_valid !== 1'b1) || pf_mshr_install) begin
                    state <= S_IDLE;
                end
            end
