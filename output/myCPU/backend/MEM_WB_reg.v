...
module MEM_WB_reg (
    ...
    always @(posedge clk) begin
        ...
        // Only clear when this stage can accept a new slot (allowIn=1) and WB is not still
        // holding the current MEM_WB output for commit (wb_stage_valid).
        end else if (!valid && allowIn && !wb_stage_valid) begin
            wb_wdata_out    <= 32'b0;
            pc_out          <= 32'b0;
            wb_reg_addr_out <= 5'b0;
            wb_op_out       <= 1'b0;
