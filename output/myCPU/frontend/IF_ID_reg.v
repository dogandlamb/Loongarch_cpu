...
module IF_ID_reg(
    input  wire                   clk,
    input  wire                   reset,
    input  wire                   cancel_sig,         // 分支或重取指冲刷
    ...
    input  wire [31:0]            tlb_vaddr_in,       // IF 输出tlb虚拟地址（IF异常槽位时取pc_req，对正常返回为0）
    input  wire                   refetch_tag_in,     // 本应是 IF 输出重取指标志，但当前由pipeline controller直接下发
...
always @(posedge clk) begin
    ...
    // 上游（IF级）无效：仅当本级可被新槽覆盖(allowIn=1)时清空。
    // 若下游反压(allowIn=0)，必须保持当前槽，避免把在途指令/PC清零。
