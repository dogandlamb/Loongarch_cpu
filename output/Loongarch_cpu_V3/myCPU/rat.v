...
module rat(
    ...
    input  wire [4:0]          raddr8_i,
    output wire                rbusy8_o,
    output wire [`ROB_W-1:0]   rnum8_o,
    input  wire [4:0]          raddr9_i,
    output wire                rbusy9_o,
    output wire [`ROB_W-1:0]   rnum9_o,
    ...
    input  wire [4:0]          raddr10_i,
    output wire                rbusy10_o,
    input  wire [4:0]          raddr11_i,
    output wire                rbusy11_o,
    ...
    input  wire                wen2_i,
    input  wire [4:0]          waddr2_i,
    input  wire [`ROB_W-1:0]   wnum2_i,
    ...
    input  wire                cmt_en2_i,
    input  wire [4:0]          cmt_addr2_i,
    input  wire [`ROB_W-1:0]   cmt_num2_i,

    input  wire                cmt_en3_i,
    input  wire [4:0]          cmt_addr3_i,
    input  wire [`ROB_W-1:0]   cmt_num3_i
);
...
assign rbusy8_o = (raddr8_i == 5'b0) ? 1'b0 : busy[raddr8_i];
assign rbusy9_o = (raddr9_i == 5'b0) ? 1'b0 : busy[raddr9_i];
...
assign rbusy10_o = (raddr10_i == 5'b0) ? 1'b0 : busy[raddr10_i];
assign rbusy11_o = (raddr11_i == 5'b0) ? 1'b0 : busy[raddr11_i];

assign rnum0_o = num[raddr0_i];
assign rnum1_o = num[raddr1_i];
assign rnum2_o = num[raddr2_i];
assign rnum3_o = num[raddr3_i];
assign rnum8_o = num[raddr8_i];
assign rnum9_o = num[raddr9_i];
    ...
    if (reset || flush_i) begin
        ...
        for (i = 1; i < 32; i = i + 1) begin
            if ((wen0_i && (waddr0_i == i[4:0])) ||
                (wen1_i && (waddr1_i == i[4:0])) ||
                (wen2_i && (waddr2_i == i[4:0]))) begin
                busy[i] <= 1'b1;
                num[i] <= (wen2_i && (waddr2_i == i[4:0])) ? wnum2_i :
                          (wen1_i && (waddr1_i == i[4:0])) ? wnum1_i : wnum0_i;
            end else if ((cmt_en0_i && (cmt_addr0_i == i[4:0]) && (cmt_num0_i == num[i])) ||
                         (cmt_en1_i && (cmt_addr1_i == i[4:0]) && (cmt_num1_i == num[i])) ||
                         (cmt_en2_i && (cmt_addr2_i == i[4:0]) && (cmt_num2_i == num[i])) ||
                         (cmt_en3_i && (cmt_addr3_i == i[4:0]) && (cmt_num3_i == num[i]))) begin
                busy[i] <= 1'b0;
            end
        end
    end
