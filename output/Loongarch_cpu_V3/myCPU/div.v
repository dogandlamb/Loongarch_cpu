...
wire [31:0] dividend_abs = (is_signed_r && dividend_r[31])
                         ? (~dividend_r + 32'd1) : dividend_r;
wire [31:0] divisor_abs  = (is_signed_r && divisor_r[31])
                         ? (~divisor_r + 32'd1) : divisor_r;
reg [5:0] dividend_clz;
reg [5:0] divisor_clz;
integer clz_i;
always @(*) begin
    dividend_clz = 6'd32;
    divisor_clz  = 6'd32;
    for (clz_i = 0; clz_i < 32; clz_i = clz_i + 1) begin
        if (dividend_abs[clz_i])
            dividend_clz = 6'd31 - clz_i;
        if (divisor_abs[clz_i])
            divisor_clz = 6'd31 - clz_i;
    end
end
wire [5:0] start_shift = divisor_clz - dividend_clz;
    ...
    if (reset) begin
        ...
        if (prep_r) begin
            ...
            if (div_zero) begin
                ...
                quotient_r  <= 32'b0;
                remainder_r <= rem_neg ? (~dividend_abs + 32'd1) : dividend_abs;
                done_r      <= 1'b1;
            end else begin
                busy_r      <= 1'b1;
                rem_r       <= dividend_abs;
            ...
            if (bit_idx_r == 6'b0) begin
                busy_r      <= 1'b0;
                quotient_r  <= quot_neg_r ? (~iter_quot + 32'd1) : iter_quot;
                remainder_r <= rem_neg_r ? (~iter_rem + 32'd1) : iter_rem;
                done_r      <= 1'b1;
            end else begin
                rem_r       <= iter_rem;
                quot_r      <= iter_quot;
