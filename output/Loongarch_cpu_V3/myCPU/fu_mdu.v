...
module fu_mdu(
    ...
    // One full cycle before the corresponding wb_valid_o.  Reservation
    // stations turn this tag into a local one-bit completion token.
    output wire                       pre_wb_valid_o,
    output wire [`ROB_W-1:0]          pre_wb_robid_o,
...
// Track the in-flight multiplier independently of mul.done.  A flush can
// arrive after mul_start but before done; clearing this bit prevents that
// stale result from writing a ROB entry that may already have been reused.
...
reg [31:0] issue_cpucfg_value;
always @(*) begin
    case (issue_src0_i[13:0])
        14'h0001: issue_cpucfg_value = 32'h0001_f1f4;
        14'h0002: issue_cpucfg_value = 32'h0000_0000;
        14'h000a: issue_cpucfg_value = 32'h0000_0005;
        14'h000b: issue_cpucfg_value = 32'h0408_0001;
        14'h000c: issue_cpucfg_value = 32'h0408_0001;
        14'h000d: issue_cpucfg_value = 32'h0000_0000;
        default:  issue_cpucfg_value = 32'h0000_0000;
    endcase
end
...
wire [31:0] fast_data = issue_is_csr                         ? csr_old
                      ...
                      : issue_is_cpucfg                      ? issue_cpucfg_value
                      : 32'b0;
...
always @(posedge clk) begin
    if (reset) begin
        ...
        if (mul_start) begin
            mul_robid_r <= issue_robid_i;
            mul_low_r <= issue_alu_op_i[`ALU_OP_MUL_W];
            mul_valid_r <= 1'b1;
            ...
            S_IDLE: begin
                if (accept) begin
                    ...
                    r_result_is_quotient <= issue_alu_op_i[`ALU_OP_DIV_W]
                                          | issue_alu_op_i[`ALU_OP_DIV_WU];
                    if (issue_is_mul) begin
                        // The multiplier has its own registered result and
                        // metadata pipeline.  Keep the shared MDU controller
                        // idle so another multiply can be accepted next cycle.
                        r_is_mul <= 1'b0;
                        state <= S_IDLE;
                    end else if (issue_is_div) begin
                        state <= S_BUSY;
                    end else begin
                        r_data  <= fast_data;
                        r_data2 <= fast_data2;
...
assign wb_valid_o = ((mul_done && mul_valid_r) || (state == S_WB)) && !flush_i;
assign wb_robid_o = mul_done ? mul_robid_r : r_robid;
assign wb_data_o  = mul_done
                  ? (mul_low_r ? mul_result[31:0] : mul_result[63:32])
                  : r_data;
assign wb_data2_o = mul_done ? 32'b0 : r_data2;

// Multiply and the fast miscellaneous operations complete one cycle after
// acceptance.  Division exposes div_done one cycle before state S_WB.  Thus
// every MDU completion has an exact, architecturally invisible pre-WB token.
wire pre_wb_div = (state == S_BUSY) && div_done && !r_is_mul;
wire pre_wb_accept_one_cycle = accept && !issue_is_div;
assign pre_wb_valid_o = (pre_wb_accept_one_cycle || pre_wb_div) && !flush_i;
assign pre_wb_robid_o = pre_wb_div ? r_robid : issue_robid_i;
