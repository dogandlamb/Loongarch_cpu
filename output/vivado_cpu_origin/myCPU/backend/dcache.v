...
wire [17:0] cache_ram_addr = (state == S_FILL) ? {10'b0, req_set}
...
wire way0_hit = (state == S_IDLE) && mem_valid && !mem_uncached && !cacop_en && way0_valid && (way0_tagv[19:0] == cur_tag);
wire way1_hit = (state == S_IDLE) && mem_valid && !mem_uncached && !cacop_en && way1_valid && (way1_tagv[19:0] == cur_tag);
wire hit = way0_hit | way1_hit;

wire cacop_way0_hit = (state == S_IDLE) && cacop_en && way0_valid && (way0_tagv[19:0] == cacop_tag);
wire cacop_way1_hit = (state == S_IDLE) && cacop_en && way1_valid && (way1_tagv[19:0] == cacop_tag);
wire cacop_hit = cacop_way0_hit | cacop_way1_hit;
wire cacop_hit_way = cacop_way1_hit;
wire cacop_dirty_hit = cacop_hit_way ? way_d_reg[cacop_set][1] : way_d_reg[cacop_set][0];

wire preld_way0_hit = (state == S_IDLE) && preld_en && !mem_valid && !cacop_en && way0_valid && (way0_tagv[19:0] == cur_tag);
wire preld_way1_hit = (state == S_IDLE) && preld_en && !mem_valid && !cacop_en && way1_valid && (way1_tagv[19:0] == cur_tag);
wire preld_hit = preld_way0_hit | preld_way1_hit;
...
assign mem_addr_ok = (state == S_IDLE) && !cacop_en;
...
wire [31:0] way0_bank0_in = fill_accept ? fill_word0 : merge_word(way0_bank0_dout, mem_wdata, mem_wstrb);
wire [31:0] way0_bank1_in = fill_accept ? fill_word1 : merge_word(way0_bank1_dout, mem_wdata, mem_wstrb);
wire [31:0] way0_bank2_in = fill_accept ? fill_word2 : merge_word(way0_bank2_dout, mem_wdata, mem_wstrb);
wire [31:0] way0_bank3_in = fill_accept ? fill_word3 : merge_word(way0_bank3_dout, mem_wdata, mem_wstrb);
wire [31:0] way1_bank0_in = fill_accept ? fill_word0 : merge_word(way1_bank0_dout, mem_wdata, mem_wstrb);
wire [31:0] way1_bank1_in = fill_accept ? fill_word1 : merge_word(way1_bank1_dout, mem_wdata, mem_wstrb);
wire [31:0] way1_bank2_in = fill_accept ? fill_word2 : merge_word(way1_bank2_dout, mem_wdata, mem_wstrb);
wire [31:0] way1_bank3_in = fill_accept ? fill_word3 : merge_word(way1_bank3_dout, mem_wdata, mem_wstrb);
...
dcache_way0_TAGV_ram u_dcache_way0_TAGV_ram(
    ...
    .ena(1'b1),
    .wea(fill_accept ? 4'hF : (invalidate_set ? 4'hF : 4'h0))
);
...
dcache_way0_Bank0_ram u_dcache_way0_Bank0_ram(.addra(cache_ram_addr), .clka(clk), .dina(way0_bank0_in), .douta(way0_bank0_dout), .ena(1'b1), .wea(fill_accept ? 4'hF : (way0_bank0_we ? mem_wstrb : 4'h0)));
dcache_way0_Bank1_ram u_dcache_way0_Bank1_ram(.addra(cache_ram_addr), .clka(clk), .dina(way0_bank1_in), .douta(way0_bank1_dout), .ena(1'b1), .wea(fill_accept ? 4'hF : (way0_bank1_we ? mem_wstrb : 4'h0)));
dcache_way0_Bank2_ram u_dcache_way0_Bank2_ram(.addra(cache_ram_addr), .clka(clk), .dina(way0_bank2_in), .douta(way0_bank2_dout), .ena(1'b1), .wea(fill_accept ? 4'hF : (way0_bank2_we ? mem_wstrb : 4'h0)));
dcache_way0_Bank3_ram u_dcache_way0_Bank3_ram(.addra(cache_ram_addr), .clka(clk), .dina(way0_bank3_in), .douta(way0_bank3_dout), .ena(1'b1), .wea(fill_accept ? 4'hF : (way0_bank3_we ? mem_wstrb : 4'h0)));
dcache_way1_Bank0_ram u_dcache_way1_Bank0_ram(.addra(cache_ram_addr), .clka(clk), .dina(way1_bank0_in), .douta(way1_bank0_dout), .ena(1'b1), .wea(fill_accept ? 4'hF : (way1_bank0_we ? mem_wstrb : 4'h0)));
dcache_way1_Bank1_ram u_dcache_way1_Bank1_ram(.addra(cache_ram_addr), .clka(clk), .dina(way1_bank1_in), .douta(way1_bank1_dout), .ena(1'b1), .wea(fill_accept ? 4'hF : (way1_bank1_we ? mem_wstrb : 4'h0)));
dcache_way1_Bank2_ram u_dcache_way1_Bank2_ram(.addra(cache_ram_addr), .clka(clk), .dina(way1_bank2_in), .douta(way1_bank2_dout), .ena(1'b1), .wea(fill_accept ? 4'hF : (way1_bank2_we ? mem_wstrb : 4'h0)));
dcache_way1_Bank3_ram u_dcache_way1_Bank3_ram(.addra(cache_ram_addr), .clka(clk), .dina(way1_bank3_in), .douta(way1_bank3_dout), .ena(1'b1), .wea(fill_accept ? 4'hF : (way1_bank3_we ? mem_wstrb : 4'h0)));
...
always @(posedge clk) begin
    if (!resetn) begin
        ...
        if (cancel_req) begin
            ...
            case (state)
                S_IDLE: begin
                    if (cacop_en && (cacop_op == `CACOP_OP_HIT_WB) && cacop_hit && cacop_dirty_hit) begin
                        req_addr <= cacop_addr;
                        req_set <= cacop_set;
                        req_tag <= cacop_tag;
                        req_way <= cacop_hit_way;
                        req_op_r <= 1'b0;
                        req_wdata <= 32'b0;
                        req_wstrb <= 4'b0;
                        uncached_r <= 1'b0;
                        victim_line <= cacop_hit_way ? way1_line : way0_line;
                        victim_tag <= cacop_hit_way ? way1_tagv[19:0] : way0_tagv[19:0];
                        req_need_refill_r <= 1'b0;
                        req_is_cacop_r <= 1'b1;
                        req_is_preld_r <= 1'b0;
                        state <= S_WB;
            ...
            if (cacop_en && (cacop_op == `CACOP_OP_IDX_INV || cacop_op == `CACOP_OP_HIT_INV)) begin
                way_d_reg[cacop_addr[11:4]] <= 2'b00;
