...
always @(*) begin
    // 4-state safe allowIn generation: treat signals as asserted only when strictly 1.
    // This prevents X-propagation from poisoning valid/allowIn and causing WB_valid='x'.
    MEM_WB_reg_allowIn  = (!reset) && ((WB_valid !== 1'b1)  || (WB_allowIn === 1'b1));
    EXE_MEM_reg_allowIn = (!reset) && ((MEM_valid !== 1'b1) || ((MEM_allowIn === 1'b1) && (MEM_WB_reg_allowIn === 1'b1)));
    ID_EXE_reg_allowIn  = (!reset) && ((EXE_valid !== 1'b1) || ((EXE_readyGo === 1'b1) && (EXE_MEM_reg_allowIn === 1'b1)));
    IF_ID_reg_allowIn   = (!reset) && (block_sig !== 1'b1)
                                       && ((ID_valid !== 1'b1) || ((ID_readyGo === 1'b1) && (ID_EXE_reg_allowIn === 1'b1)));
    ...
    if (reset) begin
        ...
        if (cancel_sig) begin
            ...
            if (IF_ID_reg_allowIn)
                ID_valid <= (IF_valid === 1'b1) && (IF_readyGo === 1'b1);
            if (ID_EXE_reg_allowIn)
                EXE_valid <= (ID_valid === 1'b1) && (ID_readyGo === 1'b1);

            if (EXE_MEM_reg_allowIn)
                MEM_valid <= (EXE_valid === 1'b1) && (EXE_readyGo === 1'b1);
        end
        ...
        if (csr_flush || wb_refetch_tag) begin
            ...
            WB_valid <= (MEM_valid === 1'b1) && (MEM_readyGo === 1'b1);
        end
    end
end
