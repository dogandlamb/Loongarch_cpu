...
// Simulation mode: always use uncached fetches.
// The cached refill path is sensitive to BRAM init/timing under XSim; uncached keeps fetch deterministic.
wire if_uncached = 1'b1;
...
wire [17:0] cache_ram_addr = (state === S_INIT) ? init_ram_addr
                        : ((state === S_FILL) ? req_ram_addr
                        : (cacop_en_safe ? cacop_ram_addr : lk_ram_addr));
...
wire way0_hit = (state === S_IDLE) && (lk_valid === 1'b1) && (lk_uncached === 1'b0) && (cacop_en_safe !== 1'b1)
                && (way0_valid === 1'b1) && (way0_tagv[19:0] === lk_tag);
wire way1_hit = (state === S_IDLE) && (lk_valid === 1'b1) && (lk_uncached === 1'b0) && (cacop_en_safe !== 1'b1)
...
// Replacement decision must align with the lookup set (lk_set), not current input,
// because tagv/data BRAMs are synchronous and dout corresponds to the registered lk_addr.
wire replace_way = (way0_valid !== 1'b1) ? 1'b0 : ((way1_valid !== 1'b1) ? 1'b1 : lru_way1[lk_set]);
wire fill_accept = (state === S_FILL) && axi_ret_valid && (uncached_r !== 1'b1);
...
// CACOP 维护期间也要取消在途读返回，避免自修改代码场景下旧指令回放。
wire cancel_req = tlb_excp_cancel_req || cacop_en_safe;

wire invalidate_set = cacop_en_safe && (cacop_op == `CACOP_OP_IDX_INV || cacop_op == `CACOP_OP_HIT_INV);
...
assign if_data_ok = hit_clean || ((state === S_FILL) && axi_ret_valid);
assign if_data = hit_clean ? hit_data : ret_data;
assign if_pc_out = hit_clean ? { lk_addr[31:2], 2'b00 } : { req_pc[31:2], 2'b00 };
...
always @(posedge clk) begin
    if (!resetn) begin
        ...
        if (state === S_INIT) begin
            ...
            if ((lk_valid === 1'b1) && (cacop_en_safe !== 1'b1)) begin
        ...
        if (cacop_en_safe && (cacop_op == `CACOP_OP_IDX_INV || cacop_op == `CACOP_OP_HIT_INV)) begin
