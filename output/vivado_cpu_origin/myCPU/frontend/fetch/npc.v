...
// ============================================================
// npc：下一 PC 组合逻辑。优先级为：
// 1) csr_redirect 非 NONE：nextpc = csr_next_pc；
// 2) br_taken：nextpc = br_target；
// 3) 命中 pc_stall（block_sig / IF_ID_reg_allowIn=0 / axi_if_busy）：nextpc = pc_cur；
// 4) 否则顺序执行：nextpc = pc_cur + 4。
// seq_pc 中加项恒 0，用于读全 br_op/valid 位宽（Lint）。解决 run linter 报错
// 内部完成「分支用 EXE PC、顺序用取指 PC」的多路选择、pc_stall 与 br_target/seq 计算。
// ============================================================
module npc(
    input  wire                   IF_valid,          // 仅参与 seq_pc 的 lint 占位加项（恒 0，不影响功能）
