...
//
// 行为（原 TODO 第二/三步）：
// - 整行输出：命中 2 拍出整行（IDLE 接受 -> LOOKUP 比对+出行），IFU 按块
//   偏移自行切指令——旧实现"按字选择输出 32bit"的通路全部删除；
// - miss：整行读 L2（2 拍 128b，ret_last 末拍），重填后 RESP 拍出行；
// - 旧架构的 redirect_stale_* / branch_line_snap / resp_echo_* /
//   cancel_resp_holdoff 等补丁【全部删除】——那是旧顺序流水"分支冲刷与
//   多拍取指纠缠"的产物；新前端 IFU 用"在途请求配对 + drop 标志"自行
