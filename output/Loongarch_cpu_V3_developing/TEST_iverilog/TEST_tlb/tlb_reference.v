...
// ------------------------------------------------------------
// 功能（新架构下完全复用，端口不变）：
// - s0 口：取指翻译查询（经 tlb_manager 由 mmu I 通道驱动）
// - s1 口：访存翻译查询（mmu D 通道）
// - 读写口/INVTLB：由 tlb_manager 在 commit 提交 TLB 维护指令时驱动
// - 双页结构（odd/even ppn），4KB 页为主
//
//TODO: 本模块零改动直接复用（已验证）。二期优化提示：
//      1) 32 项全相联比较链是时序热点——优先方案是在 tlb_manager 里加
//         l1_tlb 微表缓存（见 l1_tlb.v），本模块保持不动；
//      2) 大页（PS!=12）支持目前按 4KB 为主，跑 Linux 需要核对 PS 字段
//         匹配逻辑的完整性（页大小可变时 vppn 比较位数随 PS 变化）。
// ============================================================
module tlb
#(
    parameter TLBNUM = 16
