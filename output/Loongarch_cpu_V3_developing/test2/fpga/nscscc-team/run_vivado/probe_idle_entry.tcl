...
set outf [open [file normalize [file dirname [info script]]/probe_idle_entry_out.txt] w]

proc snap {} {
    global cpu outf
    set v0 [get_value $cpu/u_commit/debug0_valid_o]
    set p0 [scan [get_value $cpu/u_commit/debug0_pc_o] %x]
    ...
    set p1 [scan [get_value $cpu/u_commit/debug1_pc_o] %x]
    ...
    if {($v0 == 1 && $p0 >= 0x1c010280 && $p0 <= 0x1c0102b0) || ($v1 == 1 && $p1 >= 0x1c010280 && $p1 <= 0x1c0102b0)} {
        puts $outf "[current_time] C0 v=$v0 pc=[format 0x%08x $p0] i=$i0 en=$e0 r$n0=$d0 | C1 v=$v1 pc=[format 0x%08x $p1] i=$i1 en=$e1 r$n1=$d1"
        flush $outf
    }
}
